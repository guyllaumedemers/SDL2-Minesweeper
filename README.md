...
### Testing
> Vanilla project showed that #include \<filename> is still accessible in release mode when build with CMake using MSVC generator and cl.exe compiler. HOWEVER, once imgui submodule gets added to the project, the same issue arise. sdl2 lib build and works fine without conflicting with #include \<filename> so I blame imgui cmakelist and will have to investigate on policies version control to fix this conflict. 
